...
if %PROCESSOR_ARCHITECTURE% EQU AMD64 (%systemroot%\SysWOW64\Robocopy.exe "%DEST0NE%" "%DESTW0%" /S /ETA) else (%systemroot%\System32\Robocopy.exe "%DEST0NE%" "%DESTW0%" /S /ETA)
rem if %PROCESSOR_ARCHITECTURE% EQU AMD64 (%systemroot%\SysWOW64\Robocopy.exe "%DEST0NE%" "%DESTW0%" /ETA) else (%systemroot%\System32\Robocopy.exe "%DEST0NE%" "%DESTW0%" /ETA)
